...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`language_settings_updated_at_idx\` ON \`language_settings\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`language_settings_created_at_idx\` ON \`language_settings\` (\`created_at\`);`)
